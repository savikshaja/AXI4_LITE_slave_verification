class axi_scoreboard extends uvm_scoreboard;
  ...
  task run_phase(uvm_phase phase);
    ...
    axi_seq_item op_seq;
    forever begin
      // $display("[%0t] SCOREBOARD : WAITING FOR INPUT", $time);
      ip_mon.get(ip_seq);
      // $display("[%0t] SCOREBOARD : GOT INPUT", $time);
      // $display("[%0t] SCOREBOARD : WAITING FOR OUTPUT", $time);
      op_mon.get(op_seq);
      // $display("[%0t] SCOREBOARD : GOT OUTPUT", $time);
      
      if (ip_seq.AWVALID && ip_seq.WVALID)
        write_data(ip_seq, op_seq); // Task to write data to slave memory
      else
        read_data(ip_seq, op_seq);  // Task to read data
    end
  endtask
  ...
  task write_data(axi_seq_item ip_seq, axi_seq_item op_seq);
    ...
    // $display("DEBUG: AWADDR=%h exp_resp_computed=%b", ip_seq.AWADDR, exp_resp);
    ...
    // Check the output against expected response
